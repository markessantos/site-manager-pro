...
// ===== SISTEMA DE VERIFICAÇÃO DE DEPENDÊNCIA =====
const DEPENDENCY_CHECKER = {
    // ID da extensão Cookie Blocker (será definido após criação)
    COOKIE_BLOCKER_ID: 'cookie-blocker-extension-id',
    
    // Código secreto compartilhado entre as extensões
    SECRET_CODE: 'SiteManagerPro2024CookieBlocker',
    
    // Verifica se a extensão Cookie Blocker está instalada e ativa
    async checkDependency() {
        try {
            console.log('🔍 Verificando dependência Cookie Blocker...');
            
            // Método 1: Verificar via chrome.management
            ...
            // Método 2: Tentar comunicação direta
            try {
                const response = await chrome.runtime.sendMessage(
                    cookieBlocker.id, 
                    { action: 'verifyConnection', code: this.SECRET_CODE }
                );
    ...
    // Ativa o Cookie Blocker para o domínio pai
...
chrome.runtime.onInstalled.addListener(async () => {
    ...
    // Verifica dependência na instalação
    const dependencyOk = await DEPENDENCY_CHECKER.checkDependency();
    if (!dependencyOk) {
        console.warn('⚠️ Cookie Blocker não encontrado. Funcionalidade limitada.');
    }
});
...
async function handleOpenSitePopup(siteData) {
    try {
        ...
        // ===== VERIFICAÇÃO OBRIGATÓRIA DE DEPENDÊNCIA =====
        const dependencyOk = await DEPENDENCY_CHECKER.checkDependency();
        if (!dependencyOk) {
            throw new Error('Cookie Blocker não está instalado ou ativo. Instale a extensão Cookie Blocker primeiro.');
        }
        ...
        // Ativa Cookie Blocker para o domínio pai
...
// ... existing code ...
// [Todo o resto do código permanece igual - funções de configuração, monitoramento, injeção, etc.]
...
function setupNavigationMonitoring(tabId) {
    ...
    chrome.tabs.onRemoved.addListener(async (removedTabId) => {
        if (removedTabId === tabId) {
            ...
            // Desativa Cookie Blocker se não há mais abas ativas
...
// Script de controle de navegação
function controlNavigation() {
    ...
    document.addEventListener('click', function(e) {
        ...
        if (target && target.href) {
            // Remove target="_blank" para manter na mesma janela
    ...
    window.open = function(url, name, features) {
        console.log('🔄 window.open interceptado, redirecionando para janela atual');
        window.location.href = url;
        return window;
    };
    
    // Monitora mudanças de location
    let currentUrl = window.location.href;
    
    // Intercepta mudanças de window.location
