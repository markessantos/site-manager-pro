...
class SiteManagerPro {
    ...
    async checkDependencies() {
        try {
            this.setStatus('processing', 'Verificando dependências...');
            
            // ===== VERIFICAÇÃO OBRIGATÓRIA DO COOKIE BLOCKER =====
            const dependencyResponse = await chrome.runtime.sendMessage({
                action: 'checkDependency'
            });
            
            if (!dependencyResponse.dependencyOk) {
                this.setStatus('error', 'Cookie Blocker não encontrado');
                this.showError('⚠️ A extensão Cookie Blocker deve estar instalada e ativa para usar o Site Manager Pro.\n\nInstale primeiro a extensão Cookie Blocker.');
            ...
            // Verifica URL obrigatória
            await this.checkRequiredUrl();
            
        } catch (error) {
            console.error('❌ Erro ao verificar dependências:', error);
            this.setStatus('error', 'Erro na verificação de dependências');
            this.showError('Erro ao verificar dependências: ' + error.message);
            this.activateBtn.disabled = true;
        }
    }
    ...
    async activateSite() {
        try {
            ...
            if (response && response.success) {
                ...
                this.showSuccess('✅ Site aberto em popup com cookies aplicados e isolamento ativo!');
